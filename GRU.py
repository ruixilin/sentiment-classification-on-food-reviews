...
from utilsGRU import calculate_perplexity, get_ptb_dataset, get_ptb_dataset_y, Vocab
from utilsGRU import ptb_iterator, sample
...
class Config(object):
  ...
  dropout = 1#0.9
  lr = 0.01#0.001
  l2 = 0.006#0.003#0.001 and more
class RNNLM_Model(LanguageModel):

  def load_data(self, debug=False):
    """Loads starter word-vectors and train/dev/test data."""
    self.vocab = Vocab()
    self.vocab.construct(get_ptb_dataset('trainx88_1')) #???
    self.encoded_trainx = np.array(
        [self.vocab.encode(word) for word in get_ptb_dataset('trainx88_1')],
        dtype=np.int32)
    self.encoded_trainy = np.array(
        [label for label in get_ptb_dataset_y('trainy88_1')],
        dtype=np.int32)
    self.encoded_validx = np.array(
        [self.vocab.encode(word) for word in get_ptb_dataset('validx88_1')],
        dtype=np.int32)
    self.encoded_validy = np.array(
        [label for label in get_ptb_dataset_y('validy88_1')],
        dtype=np.int32)
    self.encoded_testx = np.array(
        [self.vocab.encode(word) for word in get_ptb_dataset('testx88_1')],
        dtype=np.int32)
    self.encoded_testy = np.array(
        [label for label in get_ptb_dataset_y('testy88_1')],
        dtype=np.int32)       
